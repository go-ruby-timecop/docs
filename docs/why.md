...
`go-ruby-timecop/timecop` reimplements Ruby's `timecop` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-timecop/timecop` directly, with no Ruby runtime;
...
Because `timecop` is pure Go with cgo disabled, it also compiles to **WebAssembly**
